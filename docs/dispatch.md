...
`r` restarts work that stopped with nobody driving it — `interrupted`, `error`, `escalated`, `needs_input`, `blocked` — plus a [manually dispatched](#manual-dispatch-a-worktree-no-agent) task waiting to be handed over, and a tracking parent when [coordination](configuration.md#experimental-features) is on. On anything else it says why there was nothing to do (the PR is open, the agent is live, the card is done) rather than reporting a requeue it didn't make.
