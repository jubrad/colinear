...
project's name (with `o` to open it in the tracker), or the failure — until you dismiss it. The
drafting agent runs in the daemon, so `esc` closes the popup without stopping it; it carries on in
[`:agents`](agents.md).
