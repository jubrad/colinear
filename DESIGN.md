...
```
src/index.tsx        entry: alt-screen, DEC-2026 sync-output wrapping, stderr diversion,
                     the attach loop (TUI ⇄ interactive claude/shell/editor), shutdown
...
src/core/
  ...
  store.ts           observable Map<issueId, Task> + version counter; the single source of UI truth
...
3. Smoke boot: `LINEAR_API_KEY=lin_api_dummy script -q /dev/null timeout 5 npm run dev >/dev/null 2>&1; echo $?` → 124 means it rendered and stayed alive.
4. Real verification is dogfooding against the live Linear workspace; `~/.local/state/colinear/colinear.log` catches runtime errors and diverted stderr (React warnings land there — check it when behavior is weird).
...
- store is a module singleton; version-counter subscription, not granular.
