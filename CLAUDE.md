...
The published site lives on the **`gh-pages`** branch — the main site at the root, per-pull-request previews under `previews/pr-<n>/` — and `.github/workflows/pages.yml` publishes it through the Pages deployment API when the `docs` workflow that wrote it finishes. **Not on a push to `gh-pages`**, which cannot fire: the push is made with the default `GITHUB_TOKEN` and GitHub starts no workflow run from one, and a `push` workflow is resolved from the pushed branch's own tree — `gh-pages` holds a built site and no `.github/`. It was written that way and never ran once; `workflow_run` is what actually triggers. The legacy branch builder is deliberately not used: it failed with nothing but "Page build failed" on every commit, including one that only deleted files, while the deployment API had published every time it was asked. Both are pushed by `.github/scripts/publish-site.sh`, which is why publishing main keeps `previews/` and a preview only touches its own directory. `gh-pages` is machine-managed: it is the one branch excluded from the `no branches` ruleset, because CI's token is not an admin bypass actor.
...
Screenshots (`docs/images/`) are captured from `coli demo` in a real pty — [ttyd](https://github.com/tsl0922/ttyd) on localhost driven by a browser — because a headless render harness verifies views but not daemon behaviour; the real pty is what caught the three demo bugs in PR #34. Three things bite. **Start `coli` after sizing the window** (Ink draws to the size it had at boot). **Viewport emulation drops `devicePixelRatio` to 1 while xterm's canvas stays allocated for 2x**, so the terminal draws at half scale; toggling `term.options.fontSize` does rebuild the canvas but it comes back at the same 2x ratio, so what actually fixes it is shrinking each backing store to its CSS box and repainting — `for (const c of document.querySelectorAll('canvas')) if (c.width !== c.clientWidth) { c.width = c.clientWidth; c.height = c.clientHeight; }` then `term.refresh(0, term.rows - 1)`. And **`term.paste()` renders without reaching the shell** — `term._core.coreService.triggerDataEvent(s, true)` is what sends. 1680×790 at fontSize 16 gives 183×41, the first width where the header's key grid stops truncating.
...
- No test suite; verification is typecheck + build + check + smoke + dogfooding. `bin/check` also runs the backup round trip (`src/core/backup.check.ts`), which moves a fabricated installation between two home directories — the one place a typecheck proves nothing, since the point of the feature is that the first machine can be thrown away.
