...
| `:reviews` (`pr`) | PRs waiting on **your** review (`gh search prs --review-requested=@me`, all repos your gh auth can see). `r` starts an **assisted pre-review**: colinear checks the PR out in a worktree and an agent reads the diff in context, returning an overview plus findings; progress streams onto the card while it works. Nothing is posted — `enter` opens the **review document** full-screen — the agent's write-up on one side, a conversation with that same agent on the other (`tab` switches, `j/k` scroll, `e` edits it in `$EDITOR`, and a chat turn resumes the reviewing session so the PR is still in context). `p` posts it as one GitHub review, `A` approves, `X` requests changes, `n` attaches a note that rides along, `s` hands the terminal to that review's own claude session (suspending the agent first, as on the board), `o` opens the PR, `x` cancels a running review, `R` refreshes. Approve/request-changes are plain `gh` calls (no tokens) |
