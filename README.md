...
| `:gc` (`disk`) | worktree disk: what can be reclaimed and how much. `space` picks, `a`/`n` all/none, `+`/`-` change how long finished work is kept (7d default), `x` removes. Live tasks and reviews still in play are never listed; branches and commits stay in the repo |
