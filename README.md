...
| `:board` | kanban: Queued / Triage / Working / Needs Input / PR Open / Done / Failed; `j/l` move columns, `i/k` move cards (arrows too); cards show live duration, tokens, repo, subtask progress, CI + review state; `a`/`1-9` answer, `enter` task detail, `m` **edit task** (repo, pinned PR, instructions, model; `ctrl+r` requeues), `u` dispatch sub-issues (picker), `s` attach claude, `S` shell, `x` cancel, `r` resume, `f` **force-start a blocked task** (the work runs in parallel; its blockers become merge-order dependencies that hold the PR in draft), `c` post escalation to Linear, `o` open PR, `O` open issue, `n` issues |
