...
export class PlanManager {
  ...
  async chat(id: string, text: string) {
    ...
    try {
      ...
      const prompt = plan.sessionId && agentFor(this.cfg, 'plan').sessionExists(cwd, plan.sessionId)
      ...
      const result = await runtimeFor(this.cfg, 'plan').backend.runSession({
        ...
        resume: plan.sessionId && agentFor(this.cfg, 'plan').sessionExists(cwd, plan.sessionId) ? plan.sessionId : undefined,
  ...
  async startChat(project: Project): Promise<{ worktree: string; sessionId: string; fresh: boolean } | undefined> {
    ...
    try {
      ...
      const resumable = plan.sessionId && agentFor(this.cfg, 'plan').sessionExists(worktree, plan.sessionId);
